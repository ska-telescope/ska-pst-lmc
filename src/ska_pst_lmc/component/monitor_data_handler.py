...
class MonitorDataStore(Generic[S, T]):
    ...
    This is an abstract class that other classes should extend to allow following for a contract about how to
    handle subband data.
    """
    ...
    def monitor_data(self: MonitorDataStore) -> T:
        ...
        Implementations of this should aggregate the monitoring data to be what is the current snapshot of
        data.
...
class MonitorDataHandler(Generic[S, T]):
    ...
    def __init__(
        ...
        :param monitor_data_callback: the callback to call once subband monitoring data has been merged with
            other subband data.
        """
